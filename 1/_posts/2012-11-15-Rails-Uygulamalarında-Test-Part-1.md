...
---
Rails projesi oluşturduğumuzda, Rails aynı zamanda bir test klasörü oluşturur. Bu test klasörünün içeriğini listelemek ve görmek istiyorsak komut satırına;
	$ ls -F test/
...
Fixtures, test/fixtures dizininin altında bulunur.Yeni Rails modelleri oluşturmak için çalıştırdığımızda, fixtures taslaklarıda otomatik olarak oluşturulur ve bu dizine yerleştirilir.
...
Bizim unit ve functional klasörlerimiz için Rails varsayılan tüm test/fixtures dosyalarını otomatik olarak yükler. Yükleme işlemi 3 adımda oluşur;
   - Fixture için tabloda tekabül eden mevcut veriyi kaldırır. ???
...
---
Rails 'de Unit testleri, bizim modelleri test edebilmemiz için yazılmaktadır. Bu kılavuz için Rails scaffolding kullanılmaktadır. Bu bize yerdeğiştirme, denetleyici ve tek bir işlem ile yeni bir kaynak görünümü sağlamak için model oluşturmaktadır.
...
PostTest sınıfı burada bir TestCase tanımlar çünkü; bu TestCase ActiveSupport 'tan miras alır(devralır). PostTest böylece ActiveSupport TestCase 'den tüm yöntemleri edinir.
Test:Unit test durumunda tanımlanmış 'test'(küçük harf duyarlı) ile başlayan herhangi bir yöntem sadece bir testtir. Bundan dolayı test_password, test_valid_password ve testValidPassword gibi tüm test isimleri 'TestCase' çalıştığında otomatik olarak çalışır. Rails, test metoduna bir test ismi ve bloğu ekler. Burada eklenen 'test_' öneki normal bir Test:Unit durumu oluşturur. Yani;
	test "the truth" do
  	  assert true
	end
...
???
Method isimleri altı çizili boşluklar ile vurgulanır.??
	assert true
Bu satır onaylama işlemi olarak adlandırılır. Onaylama, beklenen sonuçlar için bir nesneyi (veya ifadeyi) değerlendiren bir kod satırıdır. Örneğin bir onaylama işleminin kontrolü:
   - bu değer = o değer mi?
   - nesne sıfır mı?
...
satırını çalıştırarak tüm rake 'lere ve bunların görevlerine açıklamaları ile birlikte ulaşabiliriz.
  ...
  3.2)
---
Bir testi çalıştırmak için basit bir şekilde Ruby test durumunda ki dosya çağırılır:
...
Bir test hatasının bildirisini görebilmek için; test durumunda post_test.rb ??*
...
Çıktıdaki 'F' harfi bize başarısız olduğunu belirtir. Altında ise başarısız test adı ile birlikte bu uyarıyı görebiliriz. ??? Varsayılan onaylama mesajları hatayı belirlemede ve düzeltmede yardımcı olmak için yeterli bilgi sağlar.
...
Bu testi çalıştırdığımızda uygulama mesajını göstermektedir: ??
	1) Failure:
	test_should_not_save_post_without_title(PostTest) [/test/unit/post_test.rb:6]:
...
* Bir çok Rails geliştiricisi bu uygulamayı kullanır (TDD). Bu egzersiz, uygulamamızın her parçası için bir test paketi oluşturmada mükemmel yoldur. TDD hakkında ayrıntılı bilgiye http://andrzejonsoftware.blogspot.com/2007/05/15-tdd-steps-to-create-rails.html buradan ulaşılabilir. 
...
Not: Her test metodu uygulamasında, bir hata veya onaylama işlemi hatası aldığında durur ve test paketi sonraki yöntem ile devam eder.??? Bütün test yöntemleri alfabetik sıraya göredir.

  3.3) Unit Testine Neler Dahil Edilir?
---

????????????????

  3.4) Mevcut Uygulamalar
---
Uygulamalar testler için önem taşır. Uygulamalar aslında yapılan şeylerin kontrolünü sağlar.
Kullanılabilecek, farklı türde birçok uygulama vardır. Aşağıda rails tarafından kullanılan test kütüphanesinde ki test/unit uygulamalarının tamamı liste halinde bulunmaktadır.????
Buradaki [msg], test hata mesajını anlaşılır bir şekilde ifade etmek için isteğe bağlı kullanılan bir parametredir. Gerekli veya zorunlu değildir.
